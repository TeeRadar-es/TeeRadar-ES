...
def ejecutar_select_supabase(tabla, columnas, ordenar_por=None, page_size=1000, filtros=None):
    """
    ...
    filtros permite aplicar condiciones simples de igualdad, por ejemplo:
    {"provincia_id": 28}
    """
...
def cargar_provincias():
    """
    Carga provincias desde Supabase para el selector inicial de origen.
    """
    try:
        ...
        st.error(f"No se pudieron cargar provincias desde Supabase: {e}")
        st.stop()

    provincias = []
    for provincia in provincias_db:
        provincias.append({
            "provincia_id": provincia.get("provincia_id"),
            "provincia_ds": provincia.get("provincia_ds"),
            "provincia": provincia.get("provincia_ds"),
            "postal_cd": provincia.get("postal_cd")
        })
...
@st.cache_data(ttl=0)
def cargar_localidades(provincia_id=None):
    """
...
def consultar_campo_golfmanager_v2(campo, metodo, fecha, hora_inicio, hora_fin, jugadores, filtro_hoyos, filtro_tipo):
    ...
    def normalizar_resource(valor):
        if valor is None:
            return None
        texto = str(valor).strip()
        if not texto:
            return None
        # Supabase puede devolver numeric como 16.0 y Golfmanager como 16.
        try:
            numero = float(texto)
    ...
    def obtener_resource_item(item):
        for clave in (
            "resource", "Resource", "idResource", "IdResource", "resourceId", "ResourceId",
            "id_resource", "resource_id"
        ):
            valor = item.get(clave)
            resource = normalizar_resource(valor)
            if resource is not None:
                return resource
        return None

    recorridos_por_resource = {}
    for recorrido_valido in recorridos_validos:
        resource_recorrido = normalizar_resource(
            recorrido_valido.get("resource") or recorrido_valido.get("idresource")
        )
        if resource_recorrido is not None:
            recorridos_por_resource[resource_recorrido] = recorrido_valido

    # Si no tenemos resource configurado en ningún recorrido válido, mantenemos
    # el comportamiento anterior para no romper otros campos Golfmanager v2.
    recorrido_fallback = recorridos_validos[0]
    ...
    try:
        registrar_debug("payload", campo, recorrido_fallback, params)

        r = session.get(endpoint, params=params, headers=headers, timeout=20)
        data = r.json()

        registrar_debug("response", campo, recorrido_fallback, data)

    except Exception as e:
        registrar_debug("response", campo, recorrido_fallback, {"error": str(e)})
        return []

    items = data.get("items", [])

    if not isinstance(items, list):
        registrar_debug("response", campo, recorrido_fallback, {
    ...
    for item in items:
        ...
        resource_item = obtener_resource_item(item)
        recorrido_item = recorridos_por_resource.get(resource_item) if resource_item is not None else None

        # En campos como Villa Nueva Golf Resort, availability.json devuelve
        # recorridos mezclados. Si el item trae Resource y no coincide con los
        # recorridos configurados/filtrados, se descarta.
        if resource_item is not None:
            if recorrido_item is None:
                continue
        else:
            recorrido_item = recorrido_fallback
        ...
        clave = (
            hora,
            normalizar_resource(resource_item) or item.get("resourceName", ""),
            jugadores_disp,
            recorrido_item.get("recorrido_id")
        )

        if clave not in resultados_agrupados:
            resultados_agrupados[clave] = {
                "recorrido": recorrido_item,
                "hora": hora,
                "jugadores_disponibles": jugadores_disp,
                "tarifas": []
            }
...
with st.container(border=True):
    ...
    lista_provincias = [p["provincia"] for p in provincias]

    col_provincia, col_localidad, col_zona = st.columns([1, 2, 1])
    ...
    if provincia_seleccionada is not None:
        provincia_obj = next(
            ...
            if p["provincia"] == provincia_seleccionada
        )
        localidades = cargar_localidades(provincia_obj.get("provincia_id"))

    lista_localidades = [l["localidad"] for l in localidades]
    ...
    with col_localidad:
        localidad_seleccionada = st.selectbox(
            ...
            index=None,
            placeholder="Selecciona primero una provincia" if provincia_obj is None else "Selecciona un municipio",
            disabled=provincia_obj is None
        )
    ...
    if localidad_seleccionada is not None:
        localidad_obj_previa = next(
            ...
            if l["localidad"] == localidad_seleccionada
        )
...
if st.button("Buscar", type="primary"):
    ...
    if provincia_seleccionada is None:
        st.error("Debes seleccionar una provincia.")
        st.stop()

    if localidad_seleccionada is None:
        st.error("Debes seleccionar un municipio.")
        st.stop()
