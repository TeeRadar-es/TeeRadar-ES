...
def resolver_campo_id_db(campo):
    """
    Resuelve el campo_id real de Supabase usando el nombre del campo.
    """
    nombre = campo.get("nombre") or campo.get("campo")
    if not nombre:
        return None
...
def obtener_distancia_cacheada_db(origen_cache, campo):
    ...
    La BD almacena distancia en metros y duración en segundos como enteros.
    La app trabaja visualmente con km y minutos.
    """
    ...
    if municipio_id_db is None:
        return None

    try:
        supabase = obtener_cliente_supabase()
        query = supabase.table("cache_rutas").select(
            "cache_id,distancia_ruta_m,duracion_ruta_s,fuente,calculo_dt"
        ).eq("auto_localizado", False).eq(
            "municipio_id", municipio_id_db
        ).eq("campo_id", campo_id_db)

        if zona_id_db is None:
            query = query.is_("zona_id", "null")
        else:
            query = query.eq("zona_id", zona_id_db)
...
def guardar_distancia_cacheada_db(origen_cache, campo, distancia_m, duracion_s, fuente="ORS"):
    ...
    if municipio_id_db is None:
        if modo_debug:
            st.warning("No se pudo guardar caché: municipio no resuelto en Supabase.")
        return
    ...
    try:
        ...
        lat_destino = round(float(campo.get("lat")), 6)
        lon_destino = round(float(campo.get("lon")), 6)
    ...
    try:
        supabase = obtener_cliente_supabase()
        query = supabase.table("cache_rutas").select("cache_id").eq(
            "auto_localizado", False
        ).eq("municipio_id", municipio_id_db).eq("campo_id", campo_id_db)
        ...
        existentes = query.limit(1).execute().data
...
def obtener_clave_cache_ruta(origen_cache, campo):
    """
    Clave textual solo para debug/compatibilidad. La búsqueda real usa Supabase.
    """
    nombre = campo.get("nombre", campo.get("campo", "campo"))
    municipio = origen_cache.get("municipio_ds", origen_cache.get("localidad", "municipio")) if origen_cache else "municipio"
    zona = origen_cache.get("zona_ds", origen_cache.get("zona", "null")) if origen_cache else "null"
    return f"municipio:{municipio}|zona:{zona}|campo:{nombre}"
...
def pintar_editor_cache_distancias():
    ...
    nombres_json = {str(c.get("nombre", "")).strip().lower(): c.get("nombre") for c in campos_json}
...
if st.button("Buscar", type="primary"):
    ...
    origen_cache = {
        # IDs del JSON se mantienen solo como referencia temporal.
        # La caché Supabase resuelve los IDs reales de BD por nombre.
        "localidad_id": localidad_id,
        "zona_id": zona_id,
        "municipio_ds": localidad_obj.get("localidad"),
        "provincia_ds": localidad_obj.get("provincia"),
        "zona_ds": zona_obj.get("zona") if "zonas" in localidad_obj else None,
        "lat": lat_ref,
        "lon": lon_ref
    }
