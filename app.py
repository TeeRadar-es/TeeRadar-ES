...
def nombre_campo(campo):
    ...
    return campo.get("campo_ds") or campo.get("nombre") or campo.get("campo") or "Campo sin nombre"


def nombre_recorrido(recorrido):
    if not recorrido:
        return "Recorrido sin nombre"
    return recorrido.get("recorrido_ds") or recorrido.get("nombre") or "Recorrido"


def get_campo_lat(campo):
    return campo.get("latitud", campo.get("lat"))


def get_campo_lon(campo):
    return campo.get("longitud", campo.get("lon"))


def es_campo_activo(campo):
    return normalizar_bool(campo.get("activo_fl", campo.get("activo")), defecto=True)


def mapear_recorrido_db(recorrido):
    """
    Recorrido procedente de Supabase.
    Incluye nombres canónicos de BD y, de forma temporal, algunos alias que todavía usan
    las funciones de consulta actuales.
    """
    return {
        ...
        # Alias temporales para no reescribir todavía todos los proveedores en este paso.
        "nombre": recorrido.get("recorrido_ds"),
        "idResourceType": recorrido.get("idresourcetype"),
        "idResource": recorrido.get("idresource"),
        "18Hoyos": normalizar_bool(recorrido.get("is_18hoyos"), defecto=False),
        "18hoyos": normalizar_bool(recorrido.get("is_18hoyos"), defecto=False),
        "9Hoyos": normalizar_bool(recorrido.get("is_9hoyos"), defecto=False),
        "CampoCorto": normalizar_bool(recorrido.get("is_campo_corto"), defecto=False),
    }
...
def aplicar_metodo_principal_a_campo(campo, metodo):
    """
    Aplica el primer método activo al campo para mantener operativas las funciones
    actuales de TeeOne/Golfmanager. En el siguiente bloque refactorizaremos esas
    funciones para recibir método como objeto independiente.
    """
    if not metodo:
        return campo

    campo["metodo"] = metodo.get("metodo_cd")

    for clave in ("url_api", "url_origen_api", "id_club", "id_agente", "area"):
        if metodo.get(clave) is not None:
            campo[clave] = metodo.get(clave)

    if metodo.get("url_reserva"):
        campo["url_reserva"] = metodo.get("url_reserva")

    # Alias temporales para proveedores ya existentes.
    if metodo.get("id_vendor") is not None:
        campo["id_vendor"] = metodo.get("id_vendor")
        campo["id_vendedor"] = metodo.get("id_vendor")

    if metodo.get("id_vendor_proveedor") is not None:
        campo["id_vendedor_proveedor"] = metodo.get("id_vendor_proveedor")
...
def cargar_campos(solo_activos=True):
    ...
    for campo_db in campos_db:
        ...
        campo = {
            ...
            # Alias temporales para mantener viva la búsqueda actual mientras limpiamos proveedores.
            "nombre": campo_db.get("campo_ds"),
            "lat": convertir_decimal_a_float(campo_db.get("latitud")),
            "lon": convertir_decimal_a_float(campo_db.get("longitud")),
            "activo": normalizar_bool(campo_db.get("activo_fl"), defecto=True),
            "consultable": normalizar_bool(campo_db.get("consultable_fl"), defecto=True),
            "telefono_reserva": campo_db.get("tfno_reserva"),
            "email_reservas": campo_db.get("email_reserva"),
            "motivo_no_consultable": campo_db.get("motivo_nocons"),
            "web": campo_db.get("web_ds"),
            "telefono": campo_db.get("tfno_ds"),
            "email": campo_db.get("email_ds"),
            "Recorridos": recorridos_por_campo.get(campo_id, []),
        }

        metodos_activos = campo["metodos"]
        if metodos_activos:
            aplicar_metodo_principal_a_campo(campo, metodos_activos[0])
...
def recorrido_cumple_filtros(recorrido, filtro_hoyos, filtro_tipo):
    es_18 = normalizar_bool(recorrido.get("is_18hoyos", recorrido.get("18Hoyos", recorrido.get("18hoyos"))), defecto=False)
    es_9 = normalizar_bool(recorrido.get("is_9hoyos", recorrido.get("9Hoyos")), defecto=False)
    es_corto = normalizar_bool(recorrido.get("is_campo_corto", recorrido.get("CampoCorto")), defecto=False)
...
def construir_resultado(campo, recorrido, hora, jugadores_disp, tarifas):
    return {
        ...
        "email_reservas": campo.get("email_reserva", campo.get("email_reservas", "No disponible")),
        "telefono_reserva": campo.get("tfno_reserva", campo.get("telefono_reserva", "No disponible")),
...
def es_campo_consultable(campo):
    return normalizar_bool(campo.get("consultable_fl", campo.get("consultable")), defecto=True)
...
def construir_campo_no_consultable(campo, filtro_hoyos, filtro_tipo):
    ...
    return {
        ...
        "distancia_ruta_km": campo.get("distancia_ruta_km"),
        "duracion_ruta_min": campo.get("duracion_ruta_min"),
        "motivo_no_consultable": campo.get("motivo_nocons", campo.get("motivo_no_consultable", "No disponible")),
        "web": campo.get("web_ds", campo.get("web", "No disponible")),
        "email": campo.get("email_ds", campo.get("email", "No disponible")),
        "telefono": campo.get("tfno_ds", campo.get("telefono", "No disponible")),
...
def resolver_campo_id_db(campo):
    ...
    nombre = campo.get("campo_ds") or campo.get("nombre") or campo.get("campo")
    if not nombre:
        return None
...
def buscar_teetimes(fecha, hora_inicio, hora_fin, jugadores, filtro_hoyos, filtro_tipo, campos_seleccionados=None, lat_ref=None, lon_ref=None, radio_km=None, origen_cache=None):
    ...
    for campo in campos:
        ...
        if metodo == "teeone_v1":
            ...
            st.warning(f"Método no soportado para {nombre_campo(campo)}: {metodo}")
